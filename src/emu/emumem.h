...
using setoffset_delegate = device_delegate<void (address_space &, offs_t)>;
...
public:
	...
	// force a recomputation on the next access of a given type (read_or_write)
	void invalidate(read_or_write mode);
...
private:
	...
	memory_access_cache<Width, AddrShift, Endian> *m_next; // next cache
